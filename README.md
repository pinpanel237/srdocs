...
로컬 Git 저장소의 파일 구조와 커밋 로그를 분석하여 Markdown 형식의 개발 위키 문서(`.md`)를 자동으로 생성해주는 Python 기반 CLI 도구입니다.

A Python-based CLI tool that analyzes the file structure and Git commit history of a local Git repository to automatically generate a development Wiki document in Markdown format.
...
- **목표**: 개발자가 코드를 원격 저장소에 Push하면 백그라운드에서 감지하여 최신 코드를 pull하고 위키 빌드(`run.py`)를 자동으로 수행합니다.
- **상세**: GitHub Actions 워크플로우 템플릿을 연동하거나 Webhook/Polling 기반 경량 백엔드 API 서버를 구동하여, 커밋 push 시 중앙 호스팅 서버나 문서 페이지로 문서가 실시간 자동 갱신되는 환경을 구축합니다.
...
- **상세**: 웹 뷰어 화면에서 마크다운 및 아키텍처 다이어그램을 직접 편집하고, 이를 로컬 소스 코드의 문서 파일에 직접 반영하여 Git Commit 및 Push까지 연동하여 양방향 지식 관리를 완성할 계획입니다.
